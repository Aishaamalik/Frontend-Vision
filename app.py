...
# Initialize session state for history and page
    ...
    if st.session_state.history:
        ...
        for entry in reversed(st.session_state.history):
            with st.expander(f"{entry['timestamp']} - {entry['image_name']}"):
                ...
                with col1:
                    if st.button("View Full Details", key=f"view_{entry['timestamp']}_{entry['image_name']}"):
                        st.session_state.selected_history = entry
                with col2:
                    if st.button("Delete", key=f"delete_{entry['timestamp']}_{entry['image_name']}"):
